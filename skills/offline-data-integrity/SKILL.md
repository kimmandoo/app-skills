...
description: Use when adding or changing local document import, external sharing, persistent library state, or reader restoration where partial copies, failed writes, stale asynchronous updates, or lost access can corrupt user data.
---
...
2. Validate the source and its destination, including canonicalized archive paths and extraction boundaries. Compute identity from content bytes, not filename, source URI, or suggested share name. Reimporting identical content retains the existing entry and locator. If its managed original is missing, stage and atomically replace only that original; retain derivatives, metadata, and reader position if repair fails.
...
- Reimport identical bytes under a different filename: entry count and locator must remain unchanged. Test a different document with the same suggested name, a dot-prefixed name, and a multi-file share: no overwrite, hidden entry, partial batch, or silent auto-open unless specified. Verify that a supported file/text item remains importable when its provider also advertises an unsupported representation. After provider access ends, the owned copy must still open.
