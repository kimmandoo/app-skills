...
3. Run `flutter run` on a selected emulator/physical device and exercise the changed user path (including navigation, permissions, async error states, and restart when persistence is involved). Follow [device-verification](../device-verification/SKILL.md); do not report build success as feature success.
