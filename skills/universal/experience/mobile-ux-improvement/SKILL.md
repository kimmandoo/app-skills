...
4. Put the next valid action where people expect it; use specific labels, a visible way out, and clear ownership of destructive actions. Keep platform-appropriate navigation and the product's established typography, color, spacing, and assets. Reveal advanced options without hiding essential context. An empty screen should explain what to do next; an error should distinguish retryable from irreversible failure. If work is saved locally but not remotely, name those states separately. Never show completion before the underlying operation succeeds.
5. If a change needs motion, name its **purpose**: feedback, spatial continuity, state recognition, or preventing a jarring swap. Gauge how often it will occur. For high-frequency paths, prefer platform defaults or no extra animation; reserve conspicuous movement for infrequent, useful moments. Do not delay input, navigation, or readable content for delight. Choose the app's existing animation mechanisms and design tokens rather than transplanting framework-specific recipes into a different stack.
...
Adapted for mobile workflows from the motion-restraint and audit principles in [Emil Kowalski's skills](https://github.com/emilkowalski/skills) (MIT; [upstream license](UPSTREAM_LICENSE)). Apply the source's CSS, React Native, and Expo recipes only when they fit the consuming app's actual stack.
