...
- Agree on a latency/memory budget, crop/input ownership, cancellation policy, and a profile indicating when expensive passes run. Keep source images in the chosen imaging pipeline; move only small candidate and tile metadata across component boundaries, not pixel buffers solely to unify detection APIs.
...
2. Define tile bounds and overlap once in the upright source-image coordinate system, using a documented, measurable geometry. On multiple target platforms, use the same geometry contract if results must behave alike; a single-platform app need not add a shared module. Make adjacent tiles overlap enough to recover faces near tile edges, then tune count/span against annotated recall, precision, latency, and memory. Never treat one photo set's tile count or input cap as a default for another product.
...
4. Execute `crop → detect → release` sequentially, retaining at most the current tile input in addition to the original/preview. When inference is asynchronous, keep the input alive until that request finishes, including after timeout/cancellation; do not start the next tile after cancellation. Gate every asynchronous result against the current image/session/operation, and emit the final response once.
5. Normalize each detection relative to *its input*, then restore tile detections to the source coordinate system. For pixel boxes, use `[left / inputWidth, top / inputHeight, right / inputWidth, bottom / inputHeight]`. For a bottom-left normalized box (such as an iOS Vision result), first use `top = 1 - maxY`, `bottom = 1 - minY` (left/right unchanged). Reject non-finite values, clamp small out-of-range SDK values to `[0, 1]`, then reject non-positive-area boxes before mapping valid tile-local boxes into source coordinates:
...
6. Merge whole-image and tile candidates in source coordinates **before** expanding boxes into downstream regions, such as editable privacy masks. Reject tile-only fragments touching an internal tile edge unless supported by a duplicate from another tile or whole-image pass. Do not treat an actual source-image edge as an internal edge. Calibrate edge tolerance on annotated images.
7. Group overlapping candidates by transitive duplicate links, joining existing groups if a candidate connects them. Calibrate overlap, containment, and center-distance thresholds on annotated close-face and boundary examples; containment alone can merge distinct neighbors. Select an *actual* box rather than averaging clipped detections. Prioritize whole-image detections over cropped fragments when they are reliable, then prefer a candidate farther from a tile edge. Sort chosen boxes deterministically before assigning IDs or emitting output regions.
...
- Hand-check the synthetic mapping above; if the detector returns bottom-left normalized boxes, a local box `(minX=.2, minY=.2, maxX=.6, maxY=.6)` maps to the same source box after the y-axis flip. Run fixtures through each target detector integration and assert finite, in-range, positive-area bounds under upright and EXIF-rotated input.
...
- Inject individual pass failures, an empty-but-successful pass, timeout, cancellation, and image replacement while inference is active. Observe a single completion, partial results only from successful passes, no stale consumer update, and eventual release of the active image after inference completes.
- Record face-size-bucket recall/precision, cold/warm latency p50/p95, peak memory, and deadline rate on each target platform/device. Compare against a predeclared acceptance budget; build success alone is not quality evidence.

## Stop conditions

Stop rollout or revise measured tile geometry/merging if close faces collapse into one, boundary fragments survive, source-edge faces disappear, coordinate/orientation fixtures fail, stale callbacks alter another image, resources remain held, or precision/latency/memory budgets fail. Do not claim complete automatic detection; when detections suggest privacy regions, keep a user route to correct missed areas.
