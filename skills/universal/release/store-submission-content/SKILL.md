...
description: Prepare version-specific App Store or Google Play release copy for intended platforms, with localized claims, privacy updates, and auditable submission history.
---
...
Read the actual version source, last public version, changelog, implemented release behavior, base listings, privacy declarations, and store-console limits. Maintain a version-scoped record **for each targeted store** in the consuming repository's chosen location. Keep the base listing unchanged unless the listing itself changes; a version record contains only overrides and submission history.
...
4. Cover every locale supported by the product and reconcile meanings across translations. Keep user-visible claims consistent but store-specific wording separate.
