...
Record the review submission/build, device/OS, review message, final `Info.plist`, actual SDK/data flows, App Store Connect App Privacy answers, review notes, and availability of a physical iPhone/iPad. UMP advertising consent is not Apple's ATT authorization. Do not infer tracking from advertising alone: inspect whether app/SDK data is linked with third-party app/web data for ad targeting/measurement or shared with a data broker.

## Check the runtime gate

1. If tracking occurs, confirm the built app contains `NSUserTrackingUsageDescription` and calls `ATTrackingManager.requestTrackingAuthorization` while active. Determine whether the ATT system prompt is reachable on a fresh installation **before** UMP refresh/form and any trackable SDK/ad requests. If tracking does not occur, align SDK behavior, privacy manifests, and store answers rather than adding a misleading prompt.
2. Treat `notDetermined` (for example due to inactivity or dismissal) as pending; retry when active, not as authorized/denied. Preserve the sequence ATT decision (`authorized`/`denied`/`restricted`) → current UMP information/form → `canRequestAds` → ads SDK initialization/request. Denial must not disable core app functionality.
3. Check whether a cached UMP IDFA pre-prompt appears before ATT or remains after denial. Privacy-and-messaging console copy is configured outside the binary; do not claim a binary patch changed it. Verify no private file contents, photos, face coordinates, or file paths are passed to ads/diagnostic SDKs unless explicitly intended and disclosed.

## Reproduce and submit

On an unlocked physical test device with tracking requests allowed, reset the app's prior choice appropriately (fresh install/authorization reset); check Screen Time, managed-device, or child-account restrictions. Use the reviewed release/TestFlight configuration. Record uninterrupted fresh-launch footage: system ATT prompt, decision, any subsequent UMP form, and continued primary app flow after refusal. Redact private photos, notifications, and identifiers. Simulator footage alone does not establish physical-device review behavior.
